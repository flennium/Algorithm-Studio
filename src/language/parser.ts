...
import { spanFor, tr, type Diagnostic, type LanguageLocale, type Program, type Statement } from "./types";
...
export function parse(source: string, locale: LanguageLocale): { program?: Program; diagnostics: Diagnostic[] } {
  const lines = source.split(/\r?\n/).map((text, n) => ({ n: n + 1, text: text.replace(/\/\/.*$/, "").trim() })).filter((x) => x.text);
  ...
  const header = lines[i++]?.text.match(/^Algorithme\s+(\w+)$/iu);
  ...
  if (norm(lines[i]?.text ?? "") === "CONSTANTES") {
    ...
    while (i < lines.length && !["VARIABLES", "DEBUT"].includes(norm(lines[i].text))) {
      const line = lines[i++], match = line.text.match(/^(\w+)\s*(?:=|<-)\s*(.+)$/u);
  ...
  if (norm(lines[i]?.text ?? "") === "VARIABLES") {
    ...
    while (i < lines.length && norm(lines[i].text) !== "DEBUT") {
      ...
      else program.declarations.push(...match[1].split(",").map((x) => x.trim()));
    }
  }
  ...
  const block = (stops: string[]): Statement[] => {
    ...
    while (i < lines.length && !stops.some((stop) => norm(lines[i].text).startsWith(stop))) {
      ...
      try {
        ...
        } else if ((match = line.text.match(/^Pour\s+(\w+)\s*<-\s*(.+?)\s+[ÀA]\s+(.+?)(?:\s+Pas\s+(.+?))?\s+Faire$/iu))) {
        ...
        } else if ((match = line.text.match(/^(\w+)\s*<-\s*(.+)$/u))) body.push({ kind: "assign", name: match[1], value: expression(match[2]), line: line.n });
